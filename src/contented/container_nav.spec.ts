...
describe('TestingContainerNavCmp', () => {
    ...
    beforeEach(waitForAsync( () => {
        ...
        let contents = _.map(MockData.getContent(cnt.id, 5), c => new Content(c))
    ...
    it("Should be able to navigate by button clicks", fakeAsync(() => {
        ...
        fixture.detectChanges();
        tick(100);
        expect(parseInt(rowIdx.val(), 10)).withContext("Now we should be on the next element").toEqual(rowIdxOriginal + 2)

        prevBtn.trigger("click")
        fixture.detectChanges();
        tick(100);
        expect(parseInt(rowIdx.val(), 10)).withContext("It should go back one").toEqual(rowIdxOriginal + 1)
