...
func (as *ActionSuite) Test_ImgShouldCreatePreview() {
    ...
    media := utils.FindContent(cnt, 4, 0)
...
func (as *ActionSuite) Test_AsyncContainerPreviews() {
    ...
    cfg := utils.GetCfg()
    cfg.PreviewOverSize = 0
    previews, err := CreateContentPreviews(c_pt, media)
...
func (as *ActionSuite) Test_PreviewsWithCorrupted() {
    ...
    media, m_err := man.ListAllContent(0, 42)
    as.NoError(m_err)
    ...
    media_check, m_err := man.ListAllContent(0, 42)
    as.NoError(m_err)
