...
func SetupScreensPreview(t *testing.T) (*models.Container, models.Contents) {
	...
	assert.NoError(t, dir_err, "Did we createa preview path", t)

	empty, read_err := ioutil.ReadDir(dstPath)
	assert.NotEmpty(t, empty, "It have an empty director")
	assert.NoError(t, read_err, "It should be able to read the dst directory", t)
	return c_pt, content
}
...
func TestCfgIncExcContent(t *testing.T) {
	db := models.ResetDB(models.InitGorm(false))
...
func TestImgShouldCreatePreview(t *testing.T) {
	dir, _ := envy.MustGet("DIR")
...
func Test_CreatePreview(t *testing.T) {
...
func Test_CreateBaseT_ags(t *testing.T) {
	models.ResetDB(models.InitGorm(false))
	cfg := test_common.ResetConfig()
	...
	assert.Equal(t, len(*tags), test_common.TOTAL_TAGS, "It should keep track of what was created")

	tagsCheck, total, lErr := man.ListAllTags(TagQuery{PerPage: 50})
	assert.NoError(t, lErr, "It should not error out listing tags")
	assert.Greater(t, total, 0, "We should have a tag count")
...
func Test_CreateContainerPreviews(t *testing.T) {
	...
	assert.Greater(t, count, 0, "There should be a container")
	...
	dstPath := utils.GetContainerPreviewDst(c_pt)
	previews, read_err := ioutil.ReadDir(dstPath)
	assert.Equal(t, TOTAL_IN_SCREENS, len(previews), "It should create 6 previews")
	assert.NoError(t, read_err, "It should be able to read the directory")
	...
	for _, mc_check := range content_check {
		assert.NotEqual(t, mc_check.Preview, "", "It should now have a preview")
	}
}

func Test_AsyncContainerPreviews(t *testing.T) {
	c_pt, content := SetupScreensPreview(t)
...
func Test_PreviewAllData(t *testing.T) {
	db := models.ResetDB(models.InitGorm(false))
	...
	assert.Greater(t, count, 1, "There should be a positive count")
...
func Test_PreviewsWithCorrupted(t *testing.T) {
	...
	assert.Equal(t, 2, count, "Count should be correct")
	for _, mc := range *content {
...
func TestFindDuplicateVideos(t *testing.T) {
	...
	assert.Equal(t, test_common.TOTAL_VIDEO, total, "There should be this many videos")
	assert.Equal(t, len(*contents), total, "It should find all video content")
